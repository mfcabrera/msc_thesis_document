...
\item \textbf{Training Algorithm}: hierarchical softmax  or negative
  sampling.
\item \textbf{Additional paramters}: Discussed in section \ref{sec:w2v_other_paramters}
\end{itemize}


Table \ref{tab:word2_vec_parameters} summarizes the parameters that  need to be selected at training time
and affect the resulting word vector representations. Then comment column is
taken from the description of the paramter on the project webpage\footnote{\url{https://code.google.com/p/word2vec/}}.
...
nodes is also  approximately $\text{log}(V)$. A equivalent model replacing
...
This section describes the dditional 
...
computation time. However there is not a way to calculate this beforehand,
and this will depend on the grammatical structure of the source language of
the corpus used for learning the word representation.
...
\end{equation}
where $f(w_i)$  is the frequency of word $w_i$ and t is a threashold that can
...
\textit{min-count} words occur  \emph{before} the actual contex t creation,
the actual effective window size increase. These include words that are content-full and linearly far from the word in focus, therefore
